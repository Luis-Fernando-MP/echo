...
export const createConversation = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingConversation = await ctx.db
      .query('conversations')
      .filter(document => {
        const participants = document.field('participants')
        const argsParticipant = { ...args.participants }
        return document.or(
          document.eq(participants, argsParticipant),
          document.eq(participants, argsParticipant.reverse())
        )
      })
      .first()
...
export const getMyConversations = query({
  ...
  handler: async (ctx, args) => {
    ...
    const user = await ctx.db
      .query('users')
      .withIndex('by_tokenIdentifier', document => {
        return document.eq('tokenIdentifier', identity.tokenIdentifier)
      })
      .unique()
